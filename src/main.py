...
import os
import time
from database import init_db, log_attendance

ENCODINGS_FILE = "data/known_faces/encodings.pkl"
TOLERANCE = 0.6
LOG_COOLDOWN_SECONDS = 10  # only log the same person once per 10 seconds
...
def main():
    init_db()
    known_names_list, known_encodings_list = load_known_encodings()

    if not known_encodings_list:
        return

    last_logged = {}  # name -> timestamp of last log
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            ...
            if True in matches:
                ...
                if matches[best_match_index]:
                    ...
                    now = time.time()
                    if name not in last_logged or (now - last_logged[name]) > LOG_COOLDOWN_SECONDS:
                        log_attendance(name)
